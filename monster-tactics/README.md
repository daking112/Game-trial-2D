...
public/
  ...
  assets/
    ...
    decor/                  trees/bushes/rocks dressing the battle grid's margins
    ui/                     stitched button/panel textures (see "Art" below)
scripts/
...
rocks - `assets/decor/`) and every button/panel (`assets/ui/`) are built from
the **Tiny Swords** pack by [Pixel Frog](https://pixelfrog-assets.itch.io/tiny-swords),
via this repo's `tinyswords/` project - see `assets/ui/LICENSE.txt`. Tiny
Swords' buttons and wood-table panels ship as fragmented 3x3 nine-slice
sheets (9 separate corner/edge/center pieces, not a single flattened image);
`gen_assets.py` auto-detects each piece's bounds from its alpha channel and
recomposes them into flattened PNGs at the exact sizes this game actually
uses (`btn-large.png`, `panel-overlay.png`, etc. - rerun the script to
regenerate all of them from scratch, or to add a new size). `public/js/ui/UiKit.js`
is the one shared button/panel/link factory every scene calls into, replacing
what used to be six near-identical hand-rolled `makeButton()` copies.
